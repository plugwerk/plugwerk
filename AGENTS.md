...
- `PLUGWERK_AUTH_ENCRYPTION_KEY` — password for the AES-256-GCM (AEAD) text encryptor that protects OIDC client secrets and PASSWORD-typed application settings at rest (PBKDF2-derived 256-bit key, min 16 chars, 32+ recommended). See [ADR-0022](docs/adrs/0022-encryption-key-size.md) (key size) and [ADR-0033](docs/adrs/0033-aes-gcm-text-encryptor.md) (AES-GCM/AEAD migration)
