...
describe("UploadProgressPanel", () => {
  ...
  it("renders when files are added (panelVisible becomes true)", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("region", { name: /upload progress/i }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows file names in the entry list", () => {
    useUploadStore
      .getState()
      .addFiles([createFile("alpha.jar"), createFile("beta.zip")]);
    render(<UploadProgressPanel />);
  ...
  it("shows completion header when all uploads finish", () => {
    ...
    useUploadStore
      .getState()
      .updateEntry(a.id, { status: "success", progress: 100 });
    useUploadStore
      .getState()
      .updateEntry(b.id, { status: "success", progress: 100 });

    render(<UploadProgressPanel />);
    expect(
      screen.getByText("Upload complete — 2 succeeded"),
    ).toBeInTheDocument();
  });
  ...
  it("shows failed count in header when some uploads fail", () => {
    ...
    useUploadStore
      .getState()
      .updateEntry(b.id, { status: "failed", errorMessage: "Server error" });
  ...
  it("shows error message for failed entries", () => {
    ...
    useUploadStore
      .getState()
      .updateEntry(id, { status: "failed", errorMessage: "Duplicate version" });
