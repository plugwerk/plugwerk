...
describe("PluginCard", () => {
  ...
  it("renders the description", () => {
    renderWithRouter(<PluginCard plugin={basePlugin} namespace="acme" />);
    expect(
      screen.getByText("Handles authentication for your application."),
    ).toBeInTheDocument();
  });
  ...
  it("shows tooltip with full name when name is overflowing", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText("Auth Plugin").length).toBeGreaterThanOrEqual(
        1,
      );
    });
  });
  ...
  it("shows tooltip with full description when description is overflowing", async () => {
    ...
    const descEl = screen.getByText(
      "Handles authentication for your application.",
    );
    Object.defineProperty(descEl, "scrollHeight", {
      configurable: true,
      value: 200,
    });
    Object.defineProperty(descEl, "clientHeight", {
      configurable: true,
      value: 50,
    });
    window.dispatchEvent(new Event("resize"));
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText("Handles authentication for your application.")
          .length,
