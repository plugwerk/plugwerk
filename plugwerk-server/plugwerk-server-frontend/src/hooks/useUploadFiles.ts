  ...
  const uploadFiles = useCallback(
    async (rawFiles: readonly File[], namespace: string) => {
      ...
      const promises = newEntries.map(async (entry) => {
        ...
        try {
          await axiosInstance.post(
            `/namespaces/${namespace}/plugin-releases`,
            formData,
            {
          ...
          useUploadStore
            .getState()
            .updateEntry(entry.id, { status: "failed", errorMessage: message });
        }
      });
      ...
      const finalEntries = useUploadStore
        .getState()
        .entries.slice(-validFiles.length);
      const successCount = finalEntries.filter(
        (e) => e.status === "success",
      ).length;
      const failCount = finalEntries.filter(
        (e) => e.status === "failed",
      ).length;
