...
/**
 * Request body for transitioning a release to a new lifecycle status
 * @export
 * @interface ReleaseStatusUpdateRequest
 */
export interface ReleaseStatusUpdateRequest {
    /**
     * Target status for the release: - `published` — make the release visible in the catalog (from `draft`) - `deprecated` — mark as superseded but keep it downloadable - `yanked` — remove from catalog permanently (irreversible) 
     * @type {string}
